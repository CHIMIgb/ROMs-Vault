...
(function (global) {
    ...
    // ── Iconos por tipo — pixelarticons (pixelarticons.com / MIT) ─────────
    const ICONS = {
        success : '<svg viewBox="0 0 24 24" fill="currentColor" class="pxi pxi-alert" aria-hidden="true"><path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z"/></svg>',
        warning : '<svg viewBox="0 0 24 24" fill="currentColor" class="pxi pxi-alert" aria-hidden="true"><path d="M1 21h22L12 2 1 21zm12-3h-2v-2h2v2zm0-4h-2v-4h2v4z"/></svg>',
        danger  : '<svg viewBox="0 0 24 24" fill="currentColor" class="pxi pxi-alert" aria-hidden="true"><path d="M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z"/></svg>',
        error   : '<svg viewBox="0 0 24 24" fill="currentColor" class="pxi pxi-alert" aria-hidden="true"><path d="M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z"/></svg>',
        info    : '<svg viewBox="0 0 24 24" fill="currentColor" class="pxi pxi-alert" aria-hidden="true"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-6h2v6zm0-8h-2V7h2v2z"/></svg>',
    };
    ...
    function openConfirm({ tipo = 'info', titulo, mensaje, btnOk = 'Aceptar', btnCancel = 'Cancelar', onOk, onCancel } = {}) {
        ...
        _modalOverlay.querySelector('#rv-modal-icon').innerHTML    = ICONS[t] || ICONS.info;
    ...
    function showToast(mensaje, tipo = 'info', duracion = 3500) {
        ...
        el.innerHTML = `
            <span class="rv-toast-icon">${ICONS[t] || ICONS.info}</span>
            <span class="rv-toast-msg">${escHtml(mensaje)}</span>
            <button class="rv-toast-close" aria-label="Cerrar"><svg viewBox="0 0 24 24" fill="currentColor" class="pxi" width="12" height="12" aria-hidden="true"><path d="M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z"/></svg></button>`;
    ...
    function showInline(selector, mensaje, tipo = 'info') {
        ...
        el.innerHTML = `
            <span class="rv-inline-icon">${ICONS[t] || ICONS.info}</span>
            <span class="rv-inline-msg">${escHtml(mensaje)}</span>
            <button class="rv-inline-close" aria-label="Cerrar"><svg viewBox="0 0 24 24" fill="currentColor" class="pxi" width="12" height="12" aria-hidden="true"><path d="M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z"/></svg></button>`;
